...
import type { Student, Practice, Submission, Question, Subject, Grade, Level, Lesson } from '../lib/data'
import {
  ArrowLeft, Users, BookOpen, ClipboardList, Plus, Trash2,
  Check, X, AlertTriangle, Lock, ChevronDown, ChevronUp, Star,
  LogOut, Loader2, Mail, Layout, FolderOpen
} from 'lucide-react'
import LessonsTab from './LessonsTab'
import LibraryTab from './LibraryTab'

interface Props { setView: (v: AppView) => void }
type Tab = 'students' | 'library' | 'lessons' | 'practices' | 'reviews'
...
export default function TeacherPortal({ setView }: Props) {
  ...
  const reload = async () => {
    setLoading(true)
    try {
      const [s, l, p, sub] = await Promise.all([
        db.students.getAll(), db.lessons.getAll(),
        db.practices.getAll(), db.submissions.getAll()
      ])
      setStudents(s); setLessons(l); setPractices(p); setSubmissions(sub)
  ...
  const handleLogin = async () => {
    ...
    catch (_e) { setLoginErr('Credenciales incorrectas.') }
    finally { setLoggingIn(false) }
  }

  if (checking) return <div className="portal-root flex-center"><Loader2 size={32} className="spin"/></div>
      ...
      <div className="portal-content">
        {loading
          ...
          : <>
              ...
              {tab === 'library'   && <LibraryTab   lessons={lessons} students={students} reload={reload}/> }
              {tab === 'lessons'   && <LessonsTab   lessons={lessons} students={students} reload={reload}/>}
              {tab === 'practices' && <PracticesTab practices={practices} students={students} lessons={lessons} reload={reload}/>}
...
function StudentsTab({ students, reload }: { students: Student[], reload: () => void }) {
  ...
  const genPin = async () => {
    let pin = ''; let taken = true
    while (taken) { pin = String(Math.floor(1000 + Math.random() * 9000)); taken = await db.students.isPinTaken(pin) }
    setForm(f => ({ ...f, pin }))
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (taken) { setErr('Ese PIN ya está en uso'); return }
      await db.students.add(form); await reload()
      setShowForm(false); setForm({ firstName: '', lastName: '', grade: GRADES[0], level: LEVELS[0], pin: '' })
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="form-card">
          ...
          <div className="form-grid">
            <div className="field"><label>Nombre</label><input value={form.firstName} onChange={e => setForm(f => ({...f, firstName: e.target.value}))} placeholder="María"/></div>
            <div className="field"><label>Apellido</label><input value={form.lastName} onChange={e => setForm(f => ({...f, lastName: e.target.value}))} placeholder="Rodríguez"/></div>
            ...
            <div className="field pin-field"><label>PIN (4 dígitos)</label>
              <div className="pin-row">
                <input value={form.pin} maxLength={4} inputMode="numeric"
                  onChange={e => setForm(f => ({...f, pin: e.target.value.replace(/\D/g,'').slice(0,4)}))} placeholder="0000"/>
      ...
      <div className="student-table">
        {students.length === 0
          ? <div className="empty-state"><Users size={36}/><p>No hay alumnos registrados.</p></div>
          : students.map(s => (
            <div className="student-row" key={s.id}>
              <div className="sr-avatar">{s.firstName[0]}{s.lastName[0]}</div>
              <div className="sr-info"><strong>{s.firstName} {s.lastName}</strong><span>{s.grade} · {s.level}</span></div>
              <div className="sr-pin"><Lock size={12}/> PIN: <strong>{s.pin}</strong></div>
              <button className="icon-btn danger" onClick={async () => { if (confirm(`¿Eliminar a ${s.firstName}?`)) { await db.students.delete(s.id); reload() } }}><Trash2 size={15}/></button>
            </div>
          ))
        }
      </div>
...
// ══════════════════════════════════════════════════════════════════
// PRACTICES TAB (ahora con selector de lección)
// ══════════════════════════════════════════════════════════════════
function PracticesTab({ practices, students, lessons, reload }: { practices: Practice[], students: Student[], lessons: Lesson[], reload: () => void }) {
  const [showForm, setShowForm] = useState(false)
  const [saving, setSaving]     = useState(false)
  const [err, setErr]           = useState('')
  ...
  const addQuestion = () => {
    ...
    setForm(f => ({
      ...f,
      questions: [...f.questions, { id: uid(), text: '', points: 5, type: isMath ? 'open' : 'multiple', options: isMath ? undefined : ['','','',''], correctOption: 0 }]
    }))
  }
  ...
  const removeQ = (id: string) => setForm(f => ({ ...f, questions: f.questions.filter(q => q.id !== id) }))
  ...
  const save = async () => {
    ...
    setSaving(true); setErr('')
    try {
      await db.practices.add({ ...form, isActive: true, lessonId: form.lessonId || undefined })
      await reload(); setShowForm(false)
      setForm({ title: '', subject: SUBJECTS[0], description: '', dueDate: '', assignedTo: [], questions: [], lessonId: '' })
  ...
  return (
    <div>
        ...
        <div className="form-card">
          ...
          <div className="form-grid">
            ...
            <div className="field"><label>Materia</label>
              <select value={form.subject} onChange={e => setForm(f => ({...f, subject: e.target.value as Subject, questions: []}))}>
            ...
            <div className="field full">
              <label>Lección asociada (opcional — el alumno la verá antes de responder)</label>
              <select value={form.lessonId} onChange={e => setForm(f => ({...f, lessonId: e.target.value}))}>
                <option value="">Sin lección asociada</option>
                {lessons.filter(l => l.subject === form.subject).map(l => (
            ...
            <div className="field full"><label>Descripción</label>
              <textarea value={form.description} rows={2} onChange={e => setForm(f => ({...f, description: e.target.value}))} placeholder="Instrucciones para el alumno..."/>
            </div>
          ...
          <div className="questions-section">
            ...
            {form.subject === 'Matemáticas' && <div className="math-info"><AlertTriangle size={13}/> Matemáticas usa desarrollo abierto con anti-copiado.</div>}
                ...
                <div className="qb-header">
                  ...
                  <div className="qb-actions">
                    <div className="field-inline"><label>Pts</label>
                      <input type="number" min={1} max={100} value={q.points} style={{width:60}} onChange={e => updateQ(q.id, { points: +e.target.value })}/>
                    </div>
                    <button className="icon-btn danger sm" onClick={() => removeQ(q.id)}><Trash2 size={13}/></button>
                  </div>
                </div>
                <textarea rows={2} className="q-input" placeholder="Enunciado" value={q.text} onChange={e => updateQ(q.id, { text: e.target.value })}/>
                {q.type === 'multiple' && q.options && (
                  <div className="options-builder">
                    {q.options.map((opt, oi) => (
                      <div key={oi} className="opt-row">
                        <input type="radio" name={`correct-${q.id}`} checked={q.correctOption === oi} onChange={() => updateQ(q.id, { correctOption: oi })} title="Correcta"/>
      ...
      <div className="practice-list">
        {practices.length === 0
          ? <div className="empty-state"><BookOpen size={36}/><p>No hay prácticas creadas.</p></div>
          : practices.map(p => {
            ...
            return (
              <div className="practice-card" key={p.id}>
                <div className="pc-top">
                  ...
                  <button className="icon-btn danger sm" onClick={async () => { if(confirm('¿Eliminar práctica?')) { await db.practices.delete(p.id); reload() }}}><Trash2 size={14}/></button>
                </div>
                <h3>{p.title}</h3>
                {lesson && <p className="lesson-link-badge">📚 Lección: {lesson.title}</p>}
                <p>{p.description}</p>
                <div className="pc-meta">
                  <span>{p.questions.length} preguntas</span>
...
// REVIEWS TAB
// ══════════════════════════════════════════════════════════════════
function ReviewsTab({ submissions, practices, students, reload }: { submissions: Submission[], practices: Practice[], students: Student[], reload: () => void }) {
  ...
  const saveReview = async (sub: Submission) => {
    ...
    setSaving(sub.id)
    try { await db.submissions.update({ ...sub, reviewed: true, score, teacherNote: notes[sub.id] ?? '' }); await reload() }
  ...
  if (submissions.length === 0) return <div><h2>Revisiones</h2><div className="empty-state"><ClipboardList size={36}/><p>Ningún alumno ha entregado aún.</p></div></div>

  return (
    <div>
      <h2>Revisiones ({submissions.length})</h2>
      {pending.length > 0 && <><h3 className="review-section-title">⏳ Pendientes ({pending.length})</h3>
        {pending.map(sub => <ReviewCard key={sub.id} sub={sub} expanded={expanded} setExpanded={setExpanded} scores={scores} setScores={setScores} notes={notes} setNotes={setNotes} getStudent={getStudent} getPractice={getPractice} saveReview={saveReview} saving={saving}/>)}</>}
      {reviewed.length > 0 && <><h3 className="review-section-title">✅ Revisadas ({reviewed.length})</h3>
        {reviewed.map(sub => <ReviewCard key={sub.id} sub={sub} expanded={expanded} setExpanded={setExpanded} scores={scores} setScores={setScores} notes={notes} setNotes={setNotes} getStudent={getStudent} getPractice={getPractice} saveReview={saveReview} saving={saving}/>)}</>}
    </div>
  )
}
    ...
    <div className={`review-card ${sub.reviewed ? 'reviewed' : 'pending'}`}>
      <div className="review-header" onClick={() => setExpanded(isOpen ? null : sub.id)}>
        <div className="rh-left">
          ...
          <div><strong>{student?.firstName} {student?.lastName}</strong><span className="rh-sub">{practice?.title} · {practice?.subject}</span></div>
        </div>
        <div className="rh-right">
          {sub.reviewed ? <span className="score-badge">{sub.score}/{maxScore} pts</span> : <span className="pending-badge">Pendiente</span>}
          {sub.antiCheatFlags?.length > 0 && <span className="flag-badge"><AlertTriangle size={12}/> {sub.antiCheatFlags.length} alerta{sub.antiCheatFlags.length>1?'s':''}</span>}
      ...
      {isOpen && (
        <div className="review-body">
          ...
          {sub.antiCheatFlags?.length > 0 && <div className="cheat-warning"><AlertTriangle size={14}/><span>Alertas: <strong>{sub.antiCheatFlags.join(', ')}</strong></span></div>}
            ...
            {practice?.questions.map((q: Question, idx: number) => {
              ...
              return (
                <div className="rq-card" key={q.id}>
                  <div className="rq-header"><span>P{idx+1}: {q.text}</span><span className="rq-pts">{q.points} pts</span></div>
                  {q.type === 'multiple'
                    ...
                    : <div className="rq-answer open">{ans?.value ? String(ans.value) : <em>Sin respuesta</em>}</div>
                  }
                </div>
              )
            })}
          ...
          {!sub.reviewed && (
            <div className="review-actions">
              <div className="score-input-row">
                ...
                <input type="number" min={0} max={maxScore} value={scores[sub.id]??''}
                  onChange={e => setScores((s: any) => ({...s, [sub.id]: e.target.value}))} placeholder="0"/>
              </div>
              <div className="field full">
                <label>Nota para el alumno</label>
                <textarea rows={3} value={notes[sub.id]??''} onChange={e => setNotes((n: any) => ({...n, [sub.id]: e.target.value}))} placeholder="Retroalimentación..."/>
              </div>
              <button className="btn-primary" onClick={() => saveReview(sub)} disabled={saving===sub.id}>
                {saving===sub.id ? <><Loader2 size={14} className="spin"/> Guardando...</> : <><Check size={15}/> Guardar revisión</>}
              </button>
            </div>
          )}
          {sub.reviewed && sub.teacherNote && <div className="teacher-note"><Star size={13}/> <em>{sub.teacherNote}</em></div>}
        </div>
      )}
    </div>
  )
